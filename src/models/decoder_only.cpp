...
DecoderOnly_Model::DecoderOnly_Model(std::unique_ptr<Config> config, OrtEnv& ort_env)
  ...
  std::cout << "calling OrtSession::Create using  " << config_->model.decoder.filename << std::endl;
...
DecoderOnly_State::DecoderOnly_State(const DecoderOnly_Model& model, DeviceSpan<int32_t> sequence_lengths_unk, const GeneratorParams& params)
  ...
  if (model_.p_device_->GetType() == DeviceType::OPENVINO) {
    std::cout << "Doing something special for OpenVINO... Creating a ModelManagedKeyValueCache" << std::endl;
    kv_cache_ = std::make_shared<ModelManagedKeyValueCache>(*this);
  }
  else
  {
    kv_cache_ = std::make_shared<DefaultKeyValueCache>(*this);
  }
  input_ids_.Add();
...
void DecoderOnly_State::UpdateInputsOutputs(DeviceSpan<int32_t>& next_tokens, DeviceSpan<int32_t> beam_indices, int total_length) {
  ...
  if (model_.p_device_->GetType() == DeviceType::OPENVINO) {
    // OV Stateful models are patched in a way so that only return the logits needed for sampling.
    // For example, given 43 prompt tokens (new_length=43 here), instead of returning logits of the
    // shape [1,43,151936], it returns logits of shape [1,1,151936].
